...
TEST_F(VideoTest, Store)
{
    ...
    long test_frame_count;
    long input_frame_count;
     cv::VideoWriter testResultVideo = cv::VideoWriter("videos_tests/test_store.avi",
                    CV_FOURCC('X', 'V', 'I', 'D'),
    ...
    cv::VideoCapture test_written_video("videos_tests/test_store_store.avi");
    std::string cv_name ;
    cv_name = "cv_written_video";
...
TEST_F(VideoTest, Interval)
{
    ...
    cv::VideoCapture test_written_video("videos_tests/test_store_interval.avi");
    std::string cv_name ;
...
TEST_F(VideoTest, Resize)
{
    ...
    cv::VideoCapture test_written_video("videos_tests/test_store_resize.avi");
    std::string cv_name ;
...
TEST_F(VideoTest, Threshold)
{
   try {
    ...
    cv::VideoCapture test_written_video("videos_tests/test_store_threshold.avi");
    std::string cv_name ;
   // std::cout<< _video.get_temporary_video() <<std:endl;
     cv_name = video.get_video_id();
    cv::VideoCapture vcl_written_video(video.get_cv_video());
...
TEST_F(VideoTest, CreateUnique)
{
    try
     {
        VCL::Video video(_inputVideo); //
        video.create_unique("videos_tests/", VCL::Video::Format::MP4);
...
TEST_F(VideoTest, Crop)
{
   ...
   try {
    ...
    cv::VideoCapture test_written_video("videos_tests/test_store_crop.avi");
    // std::string cv_name ;
    // cv_name = video.get_video_id();
    cv::VideoCapture vcl_written_video(video.get_cv_video());
    input_frame_count = vcl_written_video.get(CV_CAP_PROP_FRAME_COUNT);
